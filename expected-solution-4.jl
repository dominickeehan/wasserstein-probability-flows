...
shift_distribution = Uniform(-1, 1)

repetitions = 500
history_length = 20

value_sequences = [zeros(history_length+1) for _ in 1:repetitions]
value_distributions = [[Uniform(0, 1) for _ in 1:history_length+1] for _ in 1:repetitions]
...
for repetition in 1:repetitions
    ...
    for t in 1:history_length+1
        value_distributions[repetition][t] = Uniform(μ-σ, μ+σ)
...
function parameter_fit(solve_for_weights, weight_parameters)
    ...
    plot!(1:history_length, SES_weights(1:history_length, 0.36))
...
parameter_fit(catch_WPF_weights, [1])
