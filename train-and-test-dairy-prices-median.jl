...
median_forecast_optimizer = optimizer_with_attributes(COPT.Optimizer, "Logging" => 0, "LogToConsole" => 0)

function fit_weighted_median_AR1_model(time_series, weights)
    m = length(time_series[1])

    inputs = time_series[1:end-1]
    outputs = time_series[2:end]
    transition_count = length(inputs)

    length(weights) == transition_count ||
        error("Expected $transition_count transition weights, received $(length(weights)).")

    model = Model(median_forecast_optimizer)

    @variables(model, begin
        mu[1:m]
        A[1:m, 1:m]
        residual_abs[1:transition_count, 1:m] >= 0
    end)

    for t in 1:transition_count
        for i in 1:m
            prediction = mu[i] + sum(A[i, j] * inputs[t][j] for j in 1:m)
            @constraint(model, residual_abs[t, i] >= outputs[t][i] - prediction)
            @constraint(model, residual_abs[t, i] >= prediction - outputs[t][i])
        end
    end

    @objective(model, Min,
        sum(weights[t] * residual_abs[t, i] for t in 1:transition_count for i in 1:m)
    )
    ...
    if !(status in (MOI.OPTIMAL, MOI.ALMOST_OPTIMAL))
        error("Median AR(1) LP failed to solve: termination_status=$status")
    end

    primal_status(model) == MOI.FEASIBLE_POINT ||
        error("Median AR(1) LP did not return a feasible point: primal_status=$(primal_status(model))")

    return value.(mu), value.(A)
end

function weighted_median_AR1_forecast(time_series, weights)
    mu, A = fit_weighted_median_AR1_model(time_series, weights)
    return mu + A * time_series[end]
end

loss_function(prediction, realised) = norm(prediction - realised, 1)
...
windowing_parameters = unique(ceil.(Int, LogRange(10, length(extracted_data), 30)))
...
function train_and_test_out_of_sample(parameters, solve_for_weights, d; save_cost_plot_as = nothing)
    ...
    Threads.@threads for (t, i) in ProgressBar(collect(IterTools.product(training_T:-1:1, eachindex(parameters))))
        ...
        local x = weighted_median_AR1_forecast(samples, sample_weights)
    ...
    for t in 1:testing_T
        total_parameter_costs_in_previous_stages[t] =
            vec(sum(parameter_costs[training_T+(t-1)-(parameter_tuning_window-1):training_T+(t-1), :], dims = 1))
    end
    ...
    standard_error = sem(realised_costs)

    display("Cost: $average_cost +/- $standard_error")
    ...
    if solve_for_weights == WPF_weights
        cost_plot = plot(
            ...
            xlabel = "Shift penalty, \$\\lambda\$",
            ylabel = "Average L1 cost",
            legend = nothing,
    ...
    if !(save_cost_plot_as === nothing) && !(cost_plot === nothing)
        #savefig(cost_plot, save_cost_plot_as)
    end

    return realised_costs,
        parameters[argmin(vec(sum(parameter_costs[end-(parameter_tuning_window-1):end, :], dims = 1)) / parameter_tuning_window)]
end

println("Dairy-price median-error forecasting objective")
...
function extract_results(parameters, weights, d; save_cost_plot_as = nothing)
    ...
    if save_cost_plot_as === nothing
        ...
        realised_costs, optimal_parameter =
            train_and_test_out_of_sample(parameters, weights, d; save_cost_plot_as = save_cost_plot_as)
    end
    ...
    return round(average_cost, digits = digits),
        percentage_average_difference,
        percentage_sem_difference,
        optimal_parameter
end
...
d(xi_i, xi_j) = norm(xi_i[1] - xi_j[1], 1) + norm(xi_i[2] - xi_j[2], 1)
WPF_L1_average_cost, WPF_L1_percentage_average_difference, WPF_L1_percentage_sem_difference, WPF_L1_parameter =
    extract_results(
        WPF_parameters,
        WPF_weights,
        d;
        #save_cost_plot_as = "figures/dairy-prices-median-WPF-L1-parameter-costs.pdf",
    )

WPF_L1_sample_weights =
    WPF_weights([[extracted_data[i], extracted_data[i+1]] for i in 1:length(extracted_data)-1], WPF_L1_parameter, d)
...
println("\$\\lambda\$ = $WPF_L1_parameter")

plt_probabilities = plot(
    sample_indices[WPF_L1_sample_weights .>= 1e-3],
...
#savefig(plt_probabilities, "figures/dairy-prices-median-WPF-L1-probability-assigned.pdf")

d(xi_i, xi_j) = sqrt(norm(xi_i[1] - xi_j[1], 2)^2 + norm(xi_i[2] - xi_j[2], 2)^2)
...
d(xi_i, xi_j) = max(norm(xi_i[1] - xi_j[1], Inf), norm(xi_i[2] - xi_j[2], Inf))
...
println("\\makecell[r]{Average\\\\testing L1 cost} 
