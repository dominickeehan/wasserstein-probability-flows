...
DLBA_W1_DRO_weight_parameters = [0; LogRange(1e-4, 1e0, 10)]
DLBA_W1_DRO_radius_parameters = [0; LogRange(1e-3, 1e0, 10)] #[0; LinRange(1e-2, 1e-1, 10); LinRange(2e-1, 1e-0, 10); LinRange(2e-0, 1e1, 10)]
...
function train_and_test_portfolio_out_of_sample(parameters, portfolio; plot_parameter_costs = false, save_cost_plot_as = nothing)
    ...
    wealth = 1
    for i in eachindex(realised_costs)
        wealth *= 1 - realised_costs[i]
    end
...
sample_indices = 1:length(extracted_data)
year_tick_positions = 1:(2 * 12):(length(extracted_data) + 1)
year_tick_labels = string.(2014:2:(2014 + 2 * (length(year_tick_positions) - 1)))

plt_extracted_data = plot(sample_indices,
                        100 * stack(extracted_data)',
                        xformatter = :none,
                        xticks = 1:12:(length(extracted_data) + 12),
                        xlims = (1 - 4, length(extracted_data) + 4),
                        ylabel = "Return (%)",
                        labels = nothing,
                        legend = nothing,
                        linetype = :stepmid,
...
WPF_L1_parameter_label = WPF_L1_parameter == Inf ? "\\infty" : string(round(Int, WPF_L1_parameter))
println("\$λ\$ = $WPF_L1_parameter_label")
...
plt_probabilities = plot(sample_indices[WPF_L1_sample_weights .>= 1e-3],
                ...
                xlims = (1 - 6, length(extracted_data) + 6 + 1),
